...
// Defined in src/spi/types.ts so lower-level primitives (invokeAgent, AcpSession)
// can import them without depending on this file. Re-exported here for compat.
import type { ExecResult, InteractiveExecOptions } from "./types.js";
export type { ExecResult, InteractiveExecOptions };
 ...
 * for it (`syncOut.ts`, `copyPaths` handling in `launchSandboxHandle.ts`) only ever
...
export interface NoSandboxProvider {
   ...
   *  implemented — `Sandbox.interactive()`'s default provider genuinely
   *  requires it at runtime; see the doc comment on
   *  `SandboxHandle.interactiveExec`. */
