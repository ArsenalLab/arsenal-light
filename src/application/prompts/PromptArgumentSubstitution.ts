...
/**
 * Prompt argument keys that Arsenal injects automatically.
...
/**
 * Apply `promptArgs` to a resolved prompt.
 *
 * Inline prompts pass through to the agent literally — no substitution, no
 * built-in arg injection — so supplying `promptArgs` with one fails instead
 * of being silently ignored. File prompts get the built-in args merged under
 * the caller's own, and the caller may not override a built-in key.
 */
export const applyPromptArgs = (options: {
  readonly rawPrompt: string;
  readonly isInlinePrompt: boolean;
  readonly userArgs: PromptArgs;
  readonly builtIns: {
