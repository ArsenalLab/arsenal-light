...
describe("PromptArgumentSubstitution", () => {
  ...
  const run = (
    ...
    layer: Layer.Layer<import("../../ports/Display.js").Display>,
  ) =>
    Effect.runPromise(
...
describe("applyPromptArgs", () => {
  const silent = SilentDisplay.layer(
    Ref.unsafeMake<ReadonlyArray<DisplayEntry>>([]),
  );
  const builtIns = { SOURCE_BRANCH: "feature/x", TARGET_BRANCH: "main" };
  const apply = (
    rawPrompt: string,
    isInlinePrompt: boolean,
    userArgs: Record<string, string | number | boolean> = {},
  ) =>
    Effect.runPromise(
      applyPromptArgs({ rawPrompt, isInlinePrompt, userArgs, builtIns }).pipe(
        Effect.either,
        Effect.provide(silent),
      ),
    );

  it("passes inline prompts through verbatim, placeholders included", async () => {
    const result = await apply("Work on {{SOURCE_BRANCH}}", true);
    expect(result._tag).toBe("Right");
    if (result._tag === "Right") {
      expect(result.right).toBe("Work on {{SOURCE_BRANCH}}");
    }
  });

  it("rejects promptArgs passed with an inline prompt", async () => {
    const result = await apply("hello", true, { KEY: "v" });
    expect(result._tag).toBe("Left");
    if (result._tag === "Left") expect(result.left).toBeInstanceOf(PromptError);
  });

  it("substitutes built-in and user args into file prompts", async () => {
    const result = await apply(
      "{{SOURCE_BRANCH}} -> {{TARGET_BRANCH}}: {{TASK}}",
      false,
      { TASK: "fix it" },
    );
    expect(result._tag).toBe("Right");
    if (result._tag === "Right") {
      expect(result.right).toBe("feature/x -> main: fix it");
    }
  });

  it("rejects user args that override a built-in key", async () => {
    const result = await apply("{{SOURCE_BRANCH}}", false, {
      SOURCE_BRANCH: "other",
    });
    expect(result._tag).toBe("Left");
    if (result._tag === "Left") expect(result.left).toBeInstanceOf(PromptError);
