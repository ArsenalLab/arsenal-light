...
import { makeSandboxFromHandle } from "../../ports/SandboxOps.js";
...
import { runWithHost, runWithHostExit } from "../../testing/hostProcess.js";
...
describe("syncOut", () => {
  it("extracts a single commit from sandbox back to host", async () => {
    ...
    try {
      await runWithHost(syncIn(hostDir, handle));
      ...
      await runWithHost(syncOut(hostDir, handle));
  ...
  it("successful sync-out leaves no patch artifacts in .arsenal/patches", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "host-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "initial.txt", "initial", "initial commit");
    ...
    try {
      ...
      const patchesDir = join(hostDir, ".arsenal", "patches");
  ...
  it("successful sync-out preserves tracked .arsenal files", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "host-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "initial.txt", "initial", "initial commit");

    // Commit tracked files inside .arsenal, as a real repo would have
    await mkdir(join(hostDir, ".arsenal"), { recursive: true });
    await commitFile(
      hostDir,
      join(".arsenal", "prompt.md"),
      "prompt",
      "add .arsenal config",
    );
    await commitFile(
      hostDir,
      join(".arsenal", "Dockerfile"),
      "FROM node",
      "add Dockerfile",
    );
    ...
    try {
      ...
      // The whole .arsenal dir must not be deleted by cleanup
      expect(existsSync(join(hostDir, ".arsenal", "prompt.md"))).toBe(true);
      expect(existsSync(join(hostDir, ".arsenal", "Dockerfile"))).toBe(true);
      // But the temporary patches dir should still be cleaned up
      expect(existsSync(join(hostDir, ".arsenal", "patches"))).toBe(false);
    } finally {
      await handle.close();
    }
  });
  ...
  it("failed git am preserves artifacts and prints recovery message", async () => {
    ...
    try {
      ...
      const { recoveryMessage } = await runWithHost(syncOut(hostDir, handle));
      ...
      const stderrOutput = recoveryMessage ?? "";
