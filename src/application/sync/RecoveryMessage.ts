...
export const buildRecoveryMessage = (input: RecoveryInput): string => {
  ...
  // When `branch` is set, the recovery commands assume the user is working
  // from inside .arsenal/worktree (see the preamble below), so patch paths
  // need a ../../ prefix to reach the repo root.
  ...
  // Add worktree setup preamble when a branch is given
  if (branch) {
