...
import { WorktreeError } from "../../../errors/errors.js";

/** A host path paired with where it is mounted inside the sandbox. */
 ...
 * directory when the workspace is a git worktree.
 */
export const PARENT_GIT_SANDBOX_DIR = "/.arsenal-parent-git";
 ...
 * Returns `arsenal:<dir-name>` where dir-name is the last path segment,
...
export const defaultImageName = (repoDir: string): string => {
  ...
  return `arsenal:${sanitized || "local"}`;
};
 ...
 * the Linux sandbox: a worktree's `.git` file holds a `gitdir:` pointer back
 * to the parent repo's real `.git` directory, and that pointer is a native
 * Windows path — meaningless once mounted into a Linux container.
 *
...
> =>
  Effect.gen(function* () {
    ...
    const tempDir = yield* Effect.tryPromise({
      try: () => mkdtemp(join(tmpdir(), "arsenal-git-")),
      catch: (e) =>
        new WorktreeError({
    ...
    // If the .git file wasn't in gitMounts (Arsenal-created worktree),
