...
import { ExecError, SyncError } from "../../../errors/errors.js";
import {
  withSandboxLifecycle,
  withSandboxLifecycleImpl,
} from "./SandboxLifecycle.js";
import { runHostHooks } from "./hooks.js";
import { GitClient, type GitClientService } from "../../git/GitClient.js";
import {
  nodeHostLayer,
  runWithHost,
  runWithHostExit,
...
describe("withSandboxLifecycle (worktree mode)", () => {
  const setupWorktree = async () => {
    ...
    await execAsync(
      `git worktree add -b "arsenal/test" "${worktreeDir}" HEAD`,
      { cwd: hostDir },
    );
  ...
  it("temp branch is deleted after cherry-pick", async () => {
    ...
    const { stdout } = await execAsync('git branch --list "arsenal/test"', {
      cwd: hostDir,
    });
      ...
      withSandboxLifecycle(
        ...
        (ctx) =>
          Effect.gen(function* () {
            ...
            yield* ctx.sandbox.exec(
              'sh -c "git checkout arsenal/test && git merge --no-ff feature/merge-test -m \\"Merge feature/merge-test\\""',
              { cwd: ctx.sandboxRepoDir },
            );
          }),
      ).pipe(Effect.provide(testDisplayLayer)),
      ...
      withSandboxLifecycle(
        ...
        (ctx) =>
          Effect.gen(function* () {
            ...
            yield* ctx.sandbox.exec(
              'sh -c "git checkout arsenal/test && git merge --no-ff branch-a -m \\"Merge branch-a\\""',
              { cwd: ctx.sandboxRepoDir },
            );
  ...
  it("no cherry-pick when explicit branch is given", async () => {
    ...
    expect(result.branch).toBe("arsenal/test");
    expect(result.commits).toHaveLength(1);

    // Commit is on arsenal/test, NOT cherry-picked to main
  ...
  it("logs 'No commits to sync out' when applyToHost is provided but no commits", async () => {
    ...
    const entries = await runWithHost(
      Effect.gen(function* () {
  ...
  it("retries git config lock contention, then succeeds", async () => {
    const { hostDir, worktreeDir } = await setupWorktree();

    let safeDirAttempts = 0;
    // Sandbox where the first `safe.directory` config attempt loses the race
    // for ~/.gitconfig's lock file (seen when many sandbox sessions share one
    // $HOME under no-sandbox), then succeeds.
    const sandbox: SandboxOps = {
      exec: (command, _options) =>
        Effect.sync(() => {
          if (command.includes("safe.directory")) {
            ...
            if (safeDirAttempts === 1) {
              return {
                stdout: "",
                stderr:
                  "error: could not lock config file /home/geetika/.gitconfig: File exists",
                exitCode: 255,
              };
            }
          }
  ...
  it("fails after exhausting retries on a persistent transient failure", async () => {
    ...
    // The retry schedule now backs off exponentially (up to ~8 retries) to
    // survive a real stampede of concurrent sandbox sessions — real enough
    // that this test would otherwise burn ~20s of actual wall-clock time
    // waiting it out. Fork under TestClock and fast-forward virtual time
    ...
    const program = Effect.gen(function* () {
      ...
      // A single big adjust can race ahead of the fiber: each retry only
      // registers its sleep once the *previous* attempt has failed, so
      // jumping time forward before that sleep exists advances nothing.
      // Loop with a yield between each jump so every attempt gets a chance
      // to register before the next jump.
      for (let i = 0; i < 12; i++) {
        yield* Effect.yieldNow();
...
describe("runHostHooks", () => {
  ...
  it("uses the provided cwd", async () => {
    ...
    await runWithHost(runHostHooks([{ command: "pwd > cwd.txt" }], dir));
  ...
  it("works normally when signal is not provided", async () => {
    ...
    await runWithHost(runHostHooks([{ command: "echo ok > result.txt" }], dir));
  ...
  it("respects per-hook timeoutMs override", async () => {
    ...
    await expect(
      runWithHost(runHostHooks([{ command: "sleep 2", timeoutMs: 500 }], dir)),
...
describe("withSandboxLifecycleImpl — GitClient injection", () => {
  ...
  it("propagates identity from the injected GitClient, not real host git", async () => {
    ...
    const name = await runWithHost(
      sandbox.exec("git config --global user.name", { cwd: worktreeDir }),
    );
    const email = await runWithHost(
  ...
  it("calls currentBranch, mergeBranch, and deleteBranch on the injected GitClient with the expected arguments", async () => {
    ...
    expect(mergeBranch).toHaveBeenCalledWith(hostDir, "arsenal/test", "main");
    expect(deleteBranch).toHaveBeenCalledWith(hostDir, "arsenal/test");
  });
});
