...
const runResolveEnv = (dir: string) =>
  Effect.runPromise(resolveEnv(dir).pipe(Effect.provide(NodeFileSystem)));

describe("resolveEnv", () => {
  it("returns all key-value pairs from .arsenal/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(
      join(dir, ".arsenal", ".env"),
  ...
  it("root .env is ignored even when .arsenal/.env also exists", async () => {
    const dir = await makeDir();
    ...
    expect(env["SHARED"]).toBe("sc"); // only .arsenal/.env is used
  });

  it("falls back to process.env for keys declared in .arsenal/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    // .arsenal/.env declares the key but with empty value
    await writeFile(join(dir, ".arsenal", ".env"), "MY_TOKEN=\n");
  ...
  it("does NOT pull keys from process.env that are not in .arsenal/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), "DECLARED_KEY=value\n");
  ...
  it(".arsenal/.env takes precedence over process.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), "MY_VAR=sc-val\n");
  ...
  it("ignores comments and blank lines in .arsenal/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(
      join(dir, ".arsenal", ".env"),
  ...
  it("does no validation — returns whatever keys are present in .arsenal/.env", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
  ...
  it("strips matching single quotes from values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), "TOKEN='my-token'\n");
  ...
  it("leaves mismatched quotes as-is", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), `KEY="value'\n`);
  ...
  it("leaves interior quotes as-is", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), 'KEY=some"thing\n');
  ...
  it("handles empty quoted values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), 'KEY=""\n');

    const env = await runResolveEnv(dir);
    expect(env).toEqual({});
  });

  it("process.env fallback works for keys in .arsenal/.env too", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), "FALLBACK_KEY=\n");
  ...
  it("unescapes \\n in double-quoted values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), 'KEY="line1\\nline2"\n');
  ...
  it("preserves internal whitespace in double-quoted values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), 'KEY="  spaced  "\n');
  ...
  it("handles escaped backslash before n in double-quoted values", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), 'KEY="a\\\\nb"\n');
  ...
  it("parses unquoted values unchanged", async () => {
    const dir = await makeDir();
    await mkdir(join(dir, ".arsenal"));
    await writeFile(join(dir, ".arsenal", ".env"), "KEY=plain\n");
