...
/** Find the sole worktree directory created under hostDir/.arsenal/worktrees. */
...
describe("WorktreeDockerSandboxFactory", () => {
  ...
  beforeEach(async () => {
    hostRepoDir = await mkdtemp(join(tmpdir(), "arsenal-test-"));
  ...
  it("creates a worktree on a generated arsenal/<timestamp> branch when no branch is specified", async () => {
    let observedBranch: string | undefined;
    await Effect.runPromise(
      Effect.gen(function* () {
        const factory = yield* SandboxFactory;
    ...
    expect(observedBranch).toMatch(/^arsenal\//);
  });
...
describe("WorktreeDockerSandboxFactory — isolated providers", () => {
  ...
  it("copies copyToWorktree files into the isolated sandbox via copyIn", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "arsenal-test-"));
  ...
  it("creates a worktree before starting the isolated sandbox", async () => {
    ...
    // After cleanup the worktree dir is gone, but the .arsenal/worktrees dir exists
    expect(existsSync(join(hostDir, ".arsenal", "worktrees"))).toBe(true);
  });
  ...
  it("provides hostWorktreePath in SandboxInfo", async () => {
    ...
    expect(receivedInfo!.hostWorktreePath).toContain(
      join(hostDir, ".arsenal", "worktrees"),
    );
  });
...
describe("alert helpers", () => {
  const runWithSilent = async (effect: Effect.Effect<void, never, Display>) => {
    const ref = Ref.unsafeMake<ReadonlyArray<DisplayEntry>>([]);
    await Effect.runPromise(
      effect.pipe(Effect.provide(SilentDisplay.layer(ref))),
    );
    return Effect.runPromise(Ref.get(ref));
  };

  it("alertSyncOutRecovery alerts on stderr only when a recovery message exists", async () => {
    const stderrSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    try {
      expect(await runWithSilent(alertSyncOutRecovery({}))).toEqual([]);
      expect(stderrSpy).not.toHaveBeenCalled();

      const entries = await runWithSilent(
        alertSyncOutRecovery({ recoveryMessage: "run git am --continue" }),
      );
  ...
  it("alertWorktreePreserved includes review and cleanup commands", async () => {
    const stderrSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    try {
      await runWithSilent(alertWorktreePreserved("/wt", "Worktree preserved"));
      const output = String(stderrSpy.mock.calls[0]?.[0]);
      expect(output).toContain("Worktree preserved");
      expect(output).toContain("cd /wt");
      expect(output).toContain("git worktree remove --force /wt");
    } finally {
      stderrSpy.mockRestore();
    }
  });
