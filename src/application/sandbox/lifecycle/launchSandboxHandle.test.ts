...
} from "./launchSandboxHandle.js";
import { testIsolated } from "../../../testing/test-isolated.js";
import { CopyToWorktreeTimeoutError } from "../../../errors/errors.js";
...
describe("launchSandboxHandle", () => {
  describe("bind-mount provider", () => {
    ...
    it("returns a working sandboxLayer", async () => {
      ...
      const { sandbox } = await runWithHost(
        launchSandboxHandle({
          provider,
          hostRepoDir: "/repo",
          env: {},
  ...
  describe("isolated provider", () => {
    ...
    it("creates handle, syncs repo, and returns sandboxLayer", async () => {
      ...
      const { handle, sandbox, worktreePath } = await runWithHost(
        launchSandboxHandle({
          provider,
          hostRepoDir: hostDir,
          env: {},
    ...
    it("copies copyPaths into the sandbox after sync", async () => {
      ...
      const provider = testIsolated();
      const { handle, sandbox } = await runWithHost(
        launchSandboxHandle({
          provider,
          hostRepoDir: hostDir,
          env: {},
    ...
    it("times out when copyIn hangs", async () => {
      ...
      // Fork launchSandboxHandle and advance the TestClock after syncIn completes.
    ...
    it("skips missing copyPaths without error", async () => {
      ...
      const provider = testIsolated();
      const { handle } = await runWithHost(
        launchSandboxHandle({
          provider,
          hostRepoDir: hostDir,
          env: {},
