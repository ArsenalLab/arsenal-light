...
import type { MountEntry } from "../mounts/mountUtils.js";
import { syncIn } from "../../sync/syncIn.js";
import { normalizeMounts, SANDBOX_REPO_DIR } from "../mounts/mountUtils.js";
...
/**
 * Launch a sandbox handle by dispatching on `provider.tag` — the raw,
 * handle-producing primitive. Distinct from `startSandboxAgainstTarget`
 * (`SandboxFactory.ts`), which dispatches on the higher-level provider
 * *category* (copying/hooks/mount resolution) and calls this underneath.
 *
 ...
 *
 * Returns the handle, a `SandboxOps` layer, and the worktree path.
 */
export const launchSandboxHandle = (
