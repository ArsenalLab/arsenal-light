...
import type { Display, DisplayEntry } from "../../../ports/Display.js";
import {
  create,
 ...
 * simulate someone else moving origin forward between arsenal runs.
 */
...
type TestEnv = FileSystem.FileSystem | Display | HostProcess;

/** Run an Effect and return its success value, throwing on failure. */
const run = <A, E>(effect: Effect.Effect<A, E, TestEnv>) =>
  Effect.runPromise(
    effect.pipe(Effect.provide(testLayer)) as Effect.Effect<A, never>,
  );

/** Run an Effect and return the error, throwing if it succeeds. */
const runFail = <A, E>(effect: Effect.Effect<A, E, TestEnv>) =>
  Effect.runPromise(
...
describe("generateTempBranchName", () => {
  it("returns a string in arsenal/<YYYYMMDD-HHMMSS>-<random> format", () => {
    const name = generateTempBranchName();
    expect(name).toMatch(/^arsenal\/\d{8}-\d{6}-[0-9a-f]{6}$/);
  });
  ...
  it("includes sanitized name when provided", () => {
    ...
    expect(name).toMatch(/^arsenal\/my-run\/\d{8}-\d{6}-[0-9a-f]{6}$/);
  });
...
describe("WorktreeManager.create", () => {
  it("creates a worktree at .arsenal/worktrees/<name>/", async () => {
    const repoDir = await setupRepo();
    const { path } = await run(create(repoDir));
    expect(path).toContain(join(repoDir, ".arsenal", "worktrees"));
  ...
  it("creates a arsenal/<timestamp>-<random> branch when no branch is specified", async () => {
    const repoDir = await setupRepo();
    const { branch } = await run(create(repoDir));
    expect(branch).toMatch(/^arsenal\/\d{8}-\d{6}-[0-9a-f]{6}$/);
  });

  it("includes name in branch when name is specified", async () => {
    const repoDir = await setupRepo();
    const { branch } = await run(create(repoDir, { name: "my-run" }));
    expect(branch).toMatch(/^arsenal\/my-run\/\d{8}-\d{6}-[0-9a-f]{6}$/);
  });

  it("includes name in worktree directory when name is specified", async () => {
    const repoDir = await setupRepo();
    const { path } = await run(create(repoDir, { name: "my-run" }));
    expect(path).toMatch(/arsenal-my-run-\d{8}-\d{6}-[0-9a-f]{6}$/);
  });
  ...
  it("the worktree directory is on the correct branch", async () => {
    ...
    expect(stdout.trim()).toMatch(/^arsenal\//);
  });
  ...
  it("creates a new branch from HEAD when specified branch does not exist", async () => {
    const repoDir = await setupRepo();
    const { path, branch } = await run(
      create(repoDir, { branch: "arsenal/issue-42-new-feature" }),
    );

    expect(branch).toBe("arsenal/issue-42-new-feature");
    expect(await getBranch(path)).toBe("arsenal/issue-42-new-feature");
  ...
  it("detects collision when branch is checked out in the main working tree", async () => {
    ...
    // The error should explain why this happens (arsenal uses worktrees,
  ...
  it("does not write upstream tracking config even when autoSetupMerge is enabled", async () => {
    ...
    const { path, branch } = await run(
      create(repoDir, { branch: "arsenal/no-tracking-test" }),
    );
    ...
    const { stdout: trackingConfig } = await execAsync(
      `git config --get-regexp "branch\\.arsenal/no-tracking-test\\." || true`,
      { cwd: repoDir },
    );
...
describe("WorktreeManager.pruneStale", () => {
  ...
  it("removes orphaned directories under .arsenal/worktrees/", async () => {
    const repoDir = await setupRepo();
  ...
  it("does not remove active worktrees when .arsenal is a symlink", async () => {
    // Regression test for #470: git canonicalizes worktree paths, so when
    // .arsenal is a symlink the un-canonicalized entryPath never matched
    ...
    await symlink(externalDir, join(repoDir, ".arsenal"));
