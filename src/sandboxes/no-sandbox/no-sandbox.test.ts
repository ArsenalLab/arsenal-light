...
describe("noSandbox", () => {
  ...
  describe("handle", () => {
    ...
    itPosix("exec passes env vars to spawned processes", async () => {
      ...
      const handle = await provider.create({
        worktreePath: process.cwd(),
        env: { MY_TEST_VAR: "arsenal_test_value" },
      });

      const result = await handle.exec("echo $MY_TEST_VAR");
      expect(result.stdout.trim()).toBe("arsenal_test_value");
    });

    itWindows(
      "exec passes env vars to spawned processes (cmd.exe)",
      async () => {
        // Regression test: `%VAR%` only expands when the command runs through
        // cmd.exe — if `exec` were still spawning `sh -c`, this would either
        // fail with `spawn sh ENOENT` (no `sh` on a stock Windows PATH) or
        // echo back the literal `%MY_TEST_VAR%`.
        const provider = noSandbox();
        const handle = await provider.create({
          worktreePath: process.cwd(),
          env: { MY_TEST_VAR: "arsenal_test_value" },
