...
describe("formatErrorMessage", () => {
  ...
  it("ConfigDirError passes through message (includes init hint)", () => {
    const msg = formatErrorMessage(
      new ConfigDirError({
        message: "No .arsenal/ found. Run `arsenal init` first.",
      }),
    );
    expect(msg).toContain("No .arsenal/");
    expect(msg).toContain("arsenal init");
  });
  ...
  it("AgentIdleTimeoutError passes through message", () => {
    const msg = formatErrorMessage(
      new AgentIdleTimeoutError({
        message:
          "Agent idle for 1200 seconds — no output received. Consider increasing the idleTimeoutSeconds option.",
        timeoutMs: 1_200_000,
      }),
    );
    expect(msg).toContain("1200");
    expect(msg).toContain("idleTimeoutSeconds");
  });
});
