...
import { CwdError } from "../errors/errors.js";
import { resolveCwd } from "./resolveCwd.js";

describe("resolveCwd", () => {
  it("returns process.cwd() when input is undefined", async () => {
    const result = await Effect.runPromise(
      resolveCwd(undefined).pipe(Effect.provide(NodeFileSystem)),
    );
  ...
  it("resolves a relative path against process.cwd()", async () => {
    const result = await Effect.runPromise(
      resolveCwd(".").pipe(Effect.provide(NodeFileSystem)),
    );
    expect(result).toBe(resolve(process.cwd()));
  });
  ...
  it("passes through an absolute path", async () => {
    ...
    try {
      const result = await Effect.runPromise(
        resolveCwd(dir).pipe(Effect.provide(NodeFileSystem)),
      );
  ...
  it("resolves a symlink to a directory", async () => {
    ...
    try {
      const result = await Effect.runPromise(
        resolveCwd(linkPath).pipe(Effect.provide(NodeFileSystem)),
      );
