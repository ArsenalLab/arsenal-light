...
import type { CreateSandboxOptions } from "./Sandbox.js";
import type { SandboxOps } from "../../ports/SandboxOps.js";
import {
  createBindMountSandboxProvider,
  createIsolatedSandboxProvider,
} from "../../spi/SandboxProvider.js";
import { testIsolated } from "../../testing/test-isolated.js";
...
const testSandbox = createBindMountSandboxProvider({
  ...
  create: async () => ({
    ...
    interactiveExec: createMockInteractiveExec({ output: "mock" }),
    close: async () => {},
  }),
});
  ...
  return createIsolatedSandboxProvider({
    ...
    create: async (opts) => {
      ...
      return {
        ...handle,
        exec: handle.exec,
        interactiveExec: createMockInteractiveExec((cwd) =>
...
describe("createSandbox", () => {
  it("creates a sandbox with branch and worktreePath properties", async () => {
    ...
    try {
      ...
      expect(sandbox.worktreePath).toContain(".arsenal/worktrees");
  ...
  it("sandbox.runAgent() invokes agent and returns SandboxRunResult", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
  ...
  describe("sandbox.runAgent() logging.verbose", () => {
    // One update the parser turns into text, one it drops entirely.
    const textUpdate = {
    ...
    /** Parse the log's JSON lines, ignoring human-readable output. */
    const jsonLines = (log: string): unknown[] =>
      log.split("\n").flatMap((line) => {
        try {
    ...
    const runWithLogging = async (verbose: boolean | undefined) => {
      const hostDir = await mkdtemp(join(tmpdir(), "sandbox-verbose-"));
      await initRepo(hostDir);
      await commitFile(hostDir, "init.txt", "init", "initial commit");
      const logPath = join(hostDir, "run.log");
            ...
            return {
              ...
              interactiveExec: createMockInteractiveExec({
                onPrompt: async (_prompt, client) => {
                  for (const update of [droppedUpdate, textUpdate]) {
      ...
      try {
        await sandbox.runAgent({
          agent: testProvider,
          prompt: "do something",
          maxIterations: 1,
          logging: { type: "file", path: logPath, verbose },
        });
    ...
    it("appends every raw ACP update, including dropped ones, when true", async () => {
      const updates = jsonLines(await runWithLogging(true));
      // Compare parsed: the ACP SDK re-serializes updates, so key order differs.
      expect(updates).toContainEqual(droppedUpdate);
      expect(updates).toContainEqual(textUpdate);
    });

    it("does not append raw ACP updates when unset", async () => {
      const updates = jsonLines(await runWithLogging(undefined));
      expect(updates).not.toContainEqual(droppedUpdate);
    });
  });
    ...
    const spyProvider = createBindMountSandboxProvider({
      ...
      create: async (opts) => ({
        ...
        exec: async (cmd, execOpts) => {
          // Arsenal issues a `git config --global --add safe.directory ...`
  ...
  it("reuses dirty worktree with warning", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
  ...
  it("sandbox.runAgent() returns commits made during the run", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
  ...
  it("two sequential runs with different prompts succeed on the same sandbox", async () => {
    ...
    try {
      const result1 = await sandbox.runAgent({
        agent: testProvider,
      ...
      const result2 = await sandbox.runAgent({
        agent: testProvider,
  ...
  it("provider's create() is called exactly once across multiple .runAgent() calls", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
    ...
    const spyProvider = createBindMountSandboxProvider({
      ...
      create: async (opts) => {
        ...
        return {
          ...
          interactiveExec: createMockInteractiveExec({ output: "mock output" }),
          close: async () => {
            closeCallCount++;
          },
        };
  ...
  it("isolated provider: runAgent() syncs commits to host worktree", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
  ...
  it("sandbox.interactive() collects commits made during session", async () => {
    ...
    const interactiveProvider = createBindMountSandboxProvider({
      ...
      create: async (opts) => ({
        ...
        // Simulate the agent making a commit during its turn
        interactiveExec: createMockInteractiveExec(async (cwd) => {
          await writeFile(
  ...
  it("sandbox.interactive() substitutes {{KEY}} placeholders in prompts", async () => {
    ...
    try {
      ...
      const promptArg = receivedPrompts.join("\n");
  ...
  it("sandbox.runAgent() accepts signal option (type check)", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
  ...
  it("sandbox.runAgent() rejects immediately with pre-aborted signal", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
  ...
  it("sandbox.runAgent() abort leaves handle usable for next run", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
  ...
  it("sandbox.runAgent() abort then close() works cleanly", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "sandbox-test-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "init.txt", "init", "initial commit");
  ...
  it("removes the worktree when sandbox start fails (no orphan)", async () => {
    ...
    const failingProvider = createBindMountSandboxProvider({
      name: "failing-create",
      create: async () => {
        throw new Error("Image 'arsenal:test' not found locally");
      },
    });
    ...
    try {
      ...
      const worktreesDir = join(hostDir, ".arsenal", "worktrees");
      ...
      expect(stdout).not.toContain(".arsenal/worktrees");
    } finally {
      await rm(hostDir, { recursive: true, force: true });
    }
  });
