...
import { printFileDisplayStartup } from "./runOutput.js";
import { bob } from "../../agents/bob/bob.js";
import {
  Output,
  StructuredOutputError,
} from "../../application/prompts/StructuredOutput.js";
import * as arsenal from "../../spi/SandboxProvider.js";
import { createBindMountSandboxProvider } from "../../spi/SandboxProvider.js";
import { testStubProvider } from "../../testing/test-shared.js";
...
describe("printFileDisplayStartup", () => {
  ...
  it("does not use clack (no @clack/prompts calls)", async () => {
    ...
    printFileDisplayStartup({
      logPath: "/project/.arsenal/logs/main.log",
    });
  ...
  it("shows branch name when provided", () => {
    printFileDisplayStartup({
      logPath: "/project/.arsenal/logs/main.log",
      branch: "arsenal/issue-124-file-logging",
    });
    const allOutput = consoleSpy.mock.calls.flat().join(" ");
    expect(allOutput).toContain("arsenal/issue-124-file-logging");
  });
  ...
  it("prints a relative log path when hostRepoDir equals process.cwd()", () => {
    const logPath = join(process.cwd(), ".arsenal", "logs", "main.log");
    printFileDisplayStartup({
      logPath,
      hostRepoDir: process.cwd(),
    });
    const allOutput = consoleSpy.mock.calls.flat().join(" ");
    expect(allOutput).toContain("tail -f .arsenal/logs/main.log");
  ...
  it("prints an absolute log path when hostRepoDir differs from process.cwd()", () => {
    ...
    const logPath = join(hostRepoDir, ".arsenal", "logs", "main.log");
    printFileDisplayStartup({
      logPath,
      hostRepoDir,
    });
    ...
    expect(allOutput).toContain(
      "tail -f /some/other/repo/.arsenal/logs/main.log",
    );
  });
});
...
describe("RunResult", () => {
  it("includes logFilePath when logging to a file", () => {
    const result: RunResult = {
      ...
      commits: [],
      branch: "main",
      logFilePath: "/path/to/arsenal.log",
    };
    expect(result.logFilePath).toBe("/path/to/arsenal.log");
  });
  ...
  it("carries sessionId in iterations for session-capable agent runs", () => {
    const result: RunResult = {
  ...
  it("has undefined sessionId for agents that don't report one", () => {
    const result: RunResult = {
      iterations: [{ sessionId: undefined }],
      completionSignal: undefined,
      stdout: "",
...
describe("branchStrategy on RunOptions", () => {
  it("throws when head strategy is used with an isolated provider", async () => {
    const isolatedSandbox = arsenal.createIsolatedSandboxProvider({
      name: "test-isolated",
      create: async () => ({
...
describe("buildRunSummaryRows", () => {
  ...
  it("includes sandbox name, max iterations, and branch", () => {
    const rows = buildRunSummaryRows({
      ...
      sandboxName: "docker",
      maxIterations: 5,
      branch: "arsenal/issue-160",
    });
    expect(rows["Sandbox"]).toBe("docker");
    expect(rows["Max iterations"]).toBe("5");
    expect(rows["Branch"]).toBe("arsenal/issue-160");
  });
...
describe("sanitizeBranchForFilename", () => {
  ...
  it("replaces forward slashes with dashes", () => {
    expect(sanitizeBranchForFilename("arsenal/issue-87-log-file")).toBe(
      "arsenal-issue-87-log-file",
    );
  });
  ...
  it("handles nested slashes like a typical arsenal branch", () => {
    expect(
      sanitizeBranchForFilename("arsenal/issue-87-log-file-branch-name"),
    ).toBe("arsenal-issue-87-log-file-branch-name");
  });
});
...
describe("buildLogFilename", () => {
  ...
  it("prefixes with target branch when temp branch is used", () => {
    expect(buildLogFilename("arsenal/20260325-142719", "main")).toBe(
      "main-arsenal-20260325-142719.log",
    );
  });

  it("sanitizes target branch with slashes", () => {
    expect(buildLogFilename("arsenal/20260325-142719", "feature/my-work")).toBe(
      "feature-my-work-arsenal-20260325-142719.log",
    );
  });

  it("includes agent name when branch contains agent segment", () => {
    expect(buildLogFilename("arsenal/bob/20260325-142719", "main")).toBe(
      "main-arsenal-bob-20260325-142719.log",
    );
  });
  ...
  it("appends run name after target branch prefix", () => {
    expect(
      buildLogFilename("arsenal/20260325-142719", "main", "reviewer"),
    ).toBe("main-arsenal-20260325-142719-reviewer.log");
  });
...
describe("promptFile resolution with cwd", () => {
  ...
  it("resolves relative promptFile from process.cwd(), not from cwd", async () => {
    // Regression test: promptFile must resolve against process.cwd()
    // regardless of what cwd is set to.
    const cwdDir = mkdtempSync(join(tmpdir(), "arsenal-cwd-"));
...
describe("inline prompt passthrough", () => {
  ...
  it("does not error on inline prompts that contain literal {{KEY}} text", async () => {
...
describe("run() error logging to file", () => {
  ...
  it("writes SandboxError to log file when using file logging", async () => {
    const dir = mkdtempSync(join(tmpdir(), "arsenal-run-error-"));
...
describe("formatContextWindowSize", () => {
  ...
  it("returns undefined when all token fields are zero (Bob 2.x)", () => {
    expect(
      formatContextWindowSize({
        inputTokens: 0,
        cacheCreationInputTokens: 0,
  ...
  it("uses contextTokens when set, ignoring inputTokens (ACP path)", () => {
    expect(
      formatContextWindowSize({
        inputTokens: 0,
        cacheCreationInputTokens: 0,
  ...
  it("contextTokens of 0 falls through to inputTokens sum", () => {
    expect(
      formatContextWindowSize({
        inputTokens: 10000,
        cacheCreationInputTokens: 0,
...
describe("buildContextWindowLines", () => {
  ...
  it("skips iterations where all token fields are zero (Bob 2.x)", () => {
    const lines = buildContextWindowLines([
      {
        usage: {
          inputTokens: 0,
...
describe("structured output entry-time validation", () => {
  ...
  it("validates tag presence with promptFile", async () => {
    const dir = mkdtempSync(join(tmpdir(), "arsenal-output-"));
    const promptFile = join(dir, "prompt.md");
...
describe("structured output error carries the failed session id", () => {
  ...
  // a session_id event, followed by a result line whose <result> tag holds
  // malformed JSON. Extraction then fails and the session id must survive
  ...
  const sessionEmittingSandbox = createBindMountSandboxProvider({
    ...
    create: async () => ({
      ...
      exec: async () => ({ stdout: "", stderr: "", exitCode: 0 }),
      close: async () => {},
      interactiveExec: createMockInteractiveExec({
        sessionId: "sess-abc-123",
        output: "<result>not valid json</result>",
      }),
    }),
  });

  it("threads iterations[].sessionId onto StructuredOutputError", async () => {
    // The session id is picked up from the agent's stream.
    try {
      await run({
        agent: bob("default"),
