...
describe("createWorktree", () => {
  it("creates a worktree with 'branch' strategy", async () => {
    ...
    try {
      expect(ws.worktreePath).toContain(".arsenal/worktrees");
  ...
  it("creates a worktree with 'merge-to-head' strategy", async () => {
    ...
    try {
      expect(ws.worktreePath).toContain(".arsenal/worktrees");
      expect(ws.branch).toMatch(/^arsenal\//);
      expect(existsSync(ws.worktreePath)).toBe(true);
    } finally {
      await ws.close();
      await rm(hostDir, { recursive: true, force: true });
...
describe("worktree.runAgent()", () => {
  ...
  it("runs agent and returns WorktreeRunResult", async () => {
    ...
    try {
      const result = await ws.runAgent({
        agent: bob("default"),
        sandbox,
        prompt: "do something",
        maxIterations: 1,
...
describe("worktree.attachSandbox()", () => {
  it("creates a sandbox with branch and worktreePath from the worktree", async () => {
    ...
    try {
      const sandbox = await ws.attachSandbox({
        sandbox: testSandbox,
        _test: {
  ...
  it("multiple sandboxes can be created sequentially from the same worktree", async () => {
    ...
    try {
      // First sandbox
      const sandbox1 = await ws.attachSandbox({
        sandbox: testSandbox,
        _test: {
          buildSandbox: (sandboxDir) => makeLocalSandbox(sandboxDir),
        },
      ...
      const sandbox2 = await ws.attachSandbox({
        sandbox: testSandbox,
        _test: {
          buildSandbox: (sandboxDir) => makeLocalSandbox(sandboxDir),
        },
